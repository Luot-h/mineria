...
# Intentar importar XGBoost, pero manejar si no está disponible
try:
    import xgboost as xgb
    HAS_XGBOOST = True
except ImportError:
    print("XGBoost no está instalado. Se usará solo RandomForest.")
    HAS_XGBOOST = False

# Intentar importar TensorFlow, pero no fallar si no está disponible
try:
    ...
    print("TensorFlow no está disponible. Se omitirá el entrenamiento de redes neuronales.")
...
def train_regression_model(X_train, X_test, y_train, y_test, preprocessor, model_name='driver_pay_predictor'):
    ...
    # Pipeline con preprocesamiento y modelo RandomForest
    ...
    # Inicializar variables para XGBoost
    model_xgb = None
    xgb_rmse = float('inf')
    xgb_r2 = -float('inf')
    ...
    if HAS_XGBOOST:
        ...
        try:
            ...
            tree_method = 'gpu_hist' if HAS_TENSORFLOW and len(tf.config.list_physical_devices('GPU')) > 0 else 'auto'
            ...
            print(f"Error al entrenar XGBoost: {e}")
            model_xgb = None
    
    # Elegir el mejor modelo
    if model_xgb is not None and xgb_rmse < rf_rmse:
    ...
    metrics = {
        'rf_rmse': rf_rmse,
        'rf_r2': rf_r2,
        'xgb_rmse': xgb_rmse if model_xgb is not None else None,
        'xgb_r2': xgb_r2 if model_xgb is not None else None,
...
def train_classification_model(X_train, X_test, y_train, y_test, preprocessor, model_name='trip_type_classifier'):
    ...
    # Elegir el mejor modelo
    if model_xgb is not None and xgb_accuracy > rf_accuracy:
        best_model = model_xgb
        best_model_name = "XGBoost"
    else:
        best_model = model_rf
    ...
    metrics = {
        ...
        'xgb_accuracy': xgb_accuracy if model_xgb is not None else None,
        'best_model': best_model_name
    }
...
# Función para entrenar modelo de red neuronal usando TensorFlow
def train_neural_network(X_train, X_test, y_train, y_test, preprocessor, model_name='nn_predictor', task='regression', max_epochs=30):
    """Entrena un modelo de red neuronal usando TensorFlow/Keras."""
    if not HAS_TENSORFLOW:
        print(f"TensorFlow no está disponible. Se omitirá el entrenamiento de {model_name}.")
        return None, None
    ...
    try:
        ...
        history = model.fit(
            ...
            epochs=max_epochs,  # Usar el parámetro max_epochs
            batch_size=64,
        ...
        print(f"Error al entrenar la red neuronal: {e}")
        return None, None
...
def main():
    ...
    if 'driver_pay' in df.columns:
        ...
        try:
            ...
            # Entrenar red neuronal con menos epochs (solo si TensorFlow está disponible)
            if HAS_TENSORFLOW:
                train_neural_network(X_train, X_test, y_train, y_test, preprocessor, 'driver_pay_nn', 'regression', max_epochs=20)
        except Exception as e:
    ...
    if 'to_airport' in df.columns:
        ...
        try:
            ...
            # Entrenar red neuronal con menos epochs (solo si TensorFlow está disponible)
            if HAS_TENSORFLOW:
                train_neural_network(X_train, X_test, y_train, y_test, preprocessor, 'airport_nn', 'classification', max_epochs=20)
        except Exception as e:
    ...
    if 'trip_time' in df.columns:
        ...
        try:
            ...
            # Entrenar red neuronal con menos epochs (solo si TensorFlow está disponible)
            if HAS_TENSORFLOW:
                train_neural_network(X_train, X_test, y_train, y_test, preprocessor, 'trip_time_nn', 'regression', max_epochs=20)
        except Exception as e:
...
if __name__ == "__main__":
    # Importación desde scikit-learn aquí porque podría fallar si falta el módulo
